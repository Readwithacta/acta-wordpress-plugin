 ...
 * License:     GPL-2.0+
 * Text Domain: acta-pay-per-article
 */
...
define( 'ACTA_PLUGIN_VERSION', '4.1.0' );
...
function acta_settings_init() {
    register_setting( 'acta_settings', ACTA_OPTION_KEY, array(
...
function acta_settings_page() {
    ...
    if (
        ...
        if ( empty( $email ) || empty( $first_name ) || empty( $last_name ) || empty( $country ) || empty( $currency ) || $article_price <= 0 ) {
            echo '<div class="notice notice-error"><p>' . esc_html__( 'All fields are required. Please fill in your email, name, country, currency, and a price greater than 0.', 'acta-pay-per-article' ) . '</p></div>';
        } else {
            ...
            if ( $result['success'] ) {
                ...
                echo '<div class="notice notice-success"><p>' . esc_html__( 'Successfully registered! Complete your Stripe setup below to start accepting payments.', 'acta-pay-per-article' ) . '</p></div>';
            } else {
    ...
    if (
        ...
        $conn_status = 'live';
        $stripe_url  = '';
        echo '<div class="notice notice-success"><p>' . esc_html__( 'Status updated. If your Stripe setup is complete, you\'re live!', 'acta-pay-per-article' ) . '</p></div>';
    }
    ...
    if (
        ...
        if ( $new_price > 0 ) {
            ...
            if ( $result['success'] ) {
                echo '<div class="notice notice-success"><p>' . esc_html__( 'Default price updated to ', 'acta-pay-per-article' ) . esc_html( number_format( $result['articlePrice'], 2 ) ) . '.</p></div>';
            } else {
                echo '<div class="notice notice-error"><p>' . esc_html( $result['message'] ) . '</p></div>';
            }
        }
